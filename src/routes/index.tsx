...
function Index() {
  ...
  return (
    <main className="overflow-hidden">
      ...
      <section className="py-20 sm:py-24"><div data-reveal="hidden" className="section-shell"><div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end"><div><span className="section-kicker">Our team</span><h2 className="mt-4 text-3xl font-extrabold sm:text-4xl">One mission. Many kinds of expertise.</h2></div><p className="max-w-xl leading-7 text-muted-foreground">A founding team focused on building trusted learning experiences for Somali families around the world.</p></div><div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["PT", "Product & Tech Lead"], ["OL", "Operations Lead"], ["CL", "Curriculum Lead"], ["ML", "Marketing Lead"]].map(([initials, role]) => <div key={role} className="flex items-center gap-4 rounded-lg border border-border bg-surface p-4"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-gold-soft font-display text-sm font-extrabold text-gold-foreground">{initials}</span><span className="text-sm font-extrabold">{role}</span></div>)}</div></div></section>
      ...
      <footer className="bg-primary py-12 text-primary-foreground"><div className="section-shell"><div className="grid gap-10 border-b border-primary-foreground/15 pb-10 md:grid-cols-[1.4fr_1fr_1fr]"><div><Logo inverted /><p className="mt-4 max-w-sm text-sm leading-6 text-primary-foreground/85">Bringing knowledge and opportunity to the Somali diaspora, wherever they live.</p></div><div><p className="text-xs font-extrabold uppercase tracking-[0.15em] text-gold">Explore</p><div className="mt-4 grid gap-3 text-sm text-primary-foreground/90"><a href="#courses">Courses</a><a href="#how-it-works">How it works</a><a href="#testimonials">Testimonials</a><a href={WHATSAPP_URL}>WhatsApp</a></div></div><div><p className="text-xs font-extrabold uppercase tracking-[0.15em] text-gold">Follow</p><div className="mt-4 flex gap-2"><a href="#contact" aria-label="TikTok" className="grid size-10 place-items-center rounded-md border border-primary-foreground/20"><Smartphone className="size-4" /></a><a href="#contact" aria-label="Facebook" className="grid size-10 place-items-center rounded-md border border-primary-foreground/20"><Facebook className="size-4" /></a><a href="#contact" aria-label="Instagram" className="grid size-10 place-items-center rounded-md border border-primary-foreground/20"><Instagram className="size-4" /></a></div></div></div><div className="flex flex-col gap-3 pt-6 text-xs text-primary-foreground/80 sm:flex-row sm:items-center sm:justify-between"><span>© 2026 Iftiin Academy. All rights reserved.</span><span>Pricing in USD-equivalent to protect against currency changes.</span></div></div></footer>
    </main>
  );
}