...
function Index() {
  ...
  return (
    <main className="overflow-hidden">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-border/70 bg-background/90 backdrop-blur-xl">
        <div className="section-shell grid h-17 grid-cols-[minmax(0,1fr)_auto] items-center gap-4 lg:flex lg:justify-between">
          ...
          <nav aria-label="Main navigation" className="hidden items-center gap-7 lg:flex">
            {[['Home', '#home'], ['Courses', '#courses'], ['How it works', '#how-it-works'], ['Stories', '#testimonials'], ['Contact', '#contact']].map(([label, href]) => <a key={href} href={href} className="group relative rounded-md px-2.5 py-2 text-sm font-semibold text-muted-foreground transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary/5 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span>{label}</span><span className="absolute inset-x-2.5 bottom-1 h-0.5 origin-left scale-x-0 rounded-full bg-gold transition-transform duration-300 group-hover:scale-x-100" /></a>)}
          </nav>
          <div className="hidden lg:block"><Button asChild variant="whatsapp"><a href={WHATSAPP_URL} target="_blank" rel="noreferrer" onClick={() => trackWhatsApp("Header enroll now")}><MessageCircle className="size-4 transition-transform duration-300 group-hover:rotate-[-12deg] group-hover:scale-110" />Enroll on WhatsApp <ArrowRight className="size-4 transition-transform duration-300 group-hover:translate-x-1" /></a></Button></div>
      ...
      <section id="courses" className="scroll-mt-16 bg-surface py-20 sm:py-28">
      ...
      <section id="faq" className="moon-education-bg py-20 sm:py-28 text-primary-foreground">
        ...
        <div data-reveal="hidden" className="section-shell relative z-10 grid gap-12 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          <div className="space-y-4">
            ...
            ].map(([question, answer]) => <details key={question} className="group py-5 px-3" open={question === "How does enrollment work?"}><summary className="flex cursor-pointer list-none items-center justify-between gap-6 font-display text-lg font-extrabold text-primary-foreground"><span>{question}</span><span className="grid size-8 shrink-0 place-items-center rounded-full bg-gold text-slate-950 transition-transform group-open:rotate-45">+</span></summary><p className="max-w-2xl pt-4 text-sm leading-7 text-slate-200">{answer}</p>{question === "Which course track should I choose?" && <figure className="mt-5 overflow-hidden rounded-lg border border-white/10 bg-slate-950/50"><img src={technologyImage} width={1200} height={800} loading="lazy" decoding="async" alt="Laptop displaying Microsoft Office spreadsheets and digital tools" className="h-56 w-full object-cover sm:h-72" /><figcaption className="flex items-center justify-between gap-4 px-4 py-3"><span className="text-xs font-bold uppercase tracking-[0.14em] text-gold">Microsoft Office and digital skills</span><span className="text-xs text-slate-300">Learn from your laptop</span></figcaption></figure>}</details>)}</div>
          </div>
        </div>
      </section>
      ...
      <section id="testimonials" className="moon-education-bg scroll-mt-16 py-20 text-primary-foreground sm:py-28">
        <div data-reveal="hidden" className="section-shell"><div className="max-w-2xl"><span className="section-kicker !text-gold"><Quote className="size-4" />Learner stories</span><h2 className="mt-4 text-balance text-3xl font-extrabold sm:text-5xl">Progress feels better when it feels possible.</h2></div>
          ...
          ].map(([image, name, location, quote]) => <figure key={name} className="rounded-lg border border-primary-foreground/15 bg-primary-foreground/5 p-6"><Quote className="size-7 text-gold" /><blockquote className="mt-5 min-h-28 text-base leading-7 text-primary-foreground/95">“{quote}”</blockquote><figcaption className="mt-6 flex items-center gap-3"><img src={image} width={816} height={816} loading="lazy" alt={`Illustrated placeholder avatar for ${name}`} className="size-11 rounded-full object-cover" /><span><strong className="block text-sm">{name}</strong><span className="text-xs text-primary-foreground/80">{location}</span></span></figcaption></figure>)}</div>
        </div>
      </section>
