      ...
      <section id="how-it-works" className="pattern-grid scroll-mt-16 py-20 sm:py-28">
        <div className="section-shell"><div className="mx-auto max-w-2xl text-center"><span className="section-kicker"><GraduationCap className="size-4" />How it works</span><h2 className="mt-4 text-balance text-3xl font-extrabold sm:text-5xl">Three simple steps. One brighter next chapter.</h2></div>
          <div className="relative mt-14 grid gap-5 lg:grid-cols-3">{[
            ["1", "Pick your course", "Choose the skill, test, or subject that moves your goals forward.", BookOpen],
            ["2", "Pay your way", "Use card or PayPal in the US, or EVC Plus, Zaad, and M-Pesa in Somalia and Kenya.", WalletCards],
            ["3", "Start learning", "Get enrolled, meet your instructor, and begin with live or downloadable lessons.", Play],
          ].map(([number, title, text, Icon]) => { const StepIcon = Icon as typeof BookOpen; return <article key={String(number)} className="relative rounded-lg border border-border bg-surface p-7 shadow-card"><span className="absolute right-6 top-4 font-display text-6xl font-extrabold text-muted">{number}</span><span className="grid size-12 place-items-center rounded-md bg-primary text-gold"><StepIcon className="size-5" /></span><h3 className="mt-8 text-xl font-extrabold">{title as string}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text as string}</p></article>; })}</div>
